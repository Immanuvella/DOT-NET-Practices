...
namespace Requirement3
{
...

	{
		...
		public static Dictionary<string, int> CalculateTypeCount(List<Songs> list)
		{
			var dict = new Dictionary<string, int>();
			foreach (Songs s in list)
			{
				if (dict.ContainsKey(s._songType))
				{
					dict[s._songType]++;
				}
				else
				{
					dict.Add(s._songType, 1);
				}
			}
			return dict;
		}
		...
		{
			int max = perTypeCount.Values.Max();
			string maxType=perTypeCount.FirstOrDefault(x=>x.Value==max).Key;             
				
					if(maxType=="Emotional")
					{
                       return "Depressed";
                    }
					else if(maxType =="Celebration")
					{
                        return "Happy";
                    }
					else if(maxType=="Motivational")
					{
                        return "Energetic";
                    }
					else
